...
export default async function runExecutor(
  ...
  if (!dependentsBuilt) {
    throw new Error(
      "Dependent libraries need to be built first. Try adding '--with-deps' CLI option"
    );
  }
  ...
  const npmDeps = dependencies
    .filter((dep: DependentBuildableProjectNode) => dep.node.type !== 'lib')
    .sort(
  ...
  const projectDeps = projGraph.dependencies[context.projectName].reduce(
    (result, dep) => {
